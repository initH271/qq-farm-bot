    ...
    async function visitFriend(friend, totalActions) {
        ...
        if (status.needWeed.length > 0) {
            ...
            for (const landId of status.needWeed) {
                try { await helpWeed(gid, [landId]); ok++; } catch (e) { fail++; if (isFirstFriendCheck) log('拜访', `  除草#${landId}失败: ${e.message}`); }
                await sleep(300);
            }
        ...
        if (status.needBug.length > 0) {
            ...
            for (const landId of status.needBug) {
                try { await helpInsecticide(gid, [landId]); ok++; } catch (e) { fail++; if (isFirstFriendCheck) log('拜访', `  除虫#${landId}失败: ${e.message}`); }
                await sleep(300);
            }
        ...
        if (status.needWater.length > 0) {
            ...
            for (const landId of status.needWater) {
                try { await helpWater(gid, [landId]); ok++; } catch (e) { fail++; if (isFirstFriendCheck) log('拜访', `  浇水#${landId}失败: ${e.message}`); }
                await sleep(300);
            }
